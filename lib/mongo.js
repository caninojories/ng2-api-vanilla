...
let DatabaseConnection = /** @class */ (function() {
  ...
  function DatabaseConnection(options) {
    ...
    this.options.URL = `${options.dbURL}/${options.dbName}${options.dbOptions}`;
  ...
  DatabaseConnection.getMongoConnectionOptionsFromUrl = function(url, options, skipOptions) {
    ...
    Object.keys(extractionList).map(function(optionName) {
      ...
      if (value.length > 0 && value !== url && value !== 'undefined') {
  ...
  DatabaseConnection.prototype.promiseConnection = function() {
    ...
    .getMongoConnectionOptionsFromUrl(this.options.URL, {
      useNewUrlParser: true,
      useCreateIndex: true,
      useFindAndModify: false
    });

    return this.mongoose.connect(this.options.dbURL.replace(/mongodb:\/\//, authPrefix) + `/${options.dbName}`, options)
    .then(function() {
